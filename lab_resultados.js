/* DatB compatibility loader. Runtime modules live under assets/js/. */
(function () {
    const targets = {
        ...
        'admin.js': 'assets/js/modules/admin/admin.js',
    ...
    const filename = current ? current.src.split('/').pop().split('?')[0] : '';
    const target = targets[filename];
    if (target) document.write('<script src="' + target + '"><\/script>');
})();
