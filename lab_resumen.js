...
function _lr_buildShell(user, content) {
        ...
        <div class="card-body py-3">
            <div class="d-flex gap-3 align-items-end flex-wrap">
                <div style="flex:1 1 240px;min-width:200px">
                    <label class="lr-filter-label">Laboratorio</label>
                ...
                <div>
                    <label class="lr-filter-label">Desde</label>
                    <input type="date" id="lr-from" class="form-control form-control-sm" style="min-width:148px">
                </div>
                <div>
                    <label class="lr-filter-label">Hasta</label>
                    <input type="date" id="lr-to" class="form-control form-control-sm" style="min-width:148px">
                </div>
                <button class="btn-primary-custom" id="lr-apply" style="padding:.42rem 1rem;font-size:.84rem">
                    <i class="bi bi-funnel"></i> Aplicar
                </button>
                <button class="btn-secondary-custom" id="lr-clear" style="padding:.42rem 1rem;font-size:.84rem">
    ...
    if (labs.length === 1) {
        ...
        document.getElementById('lr-lab-input').value =
            `${l.nombre}${l.nivel_referencia ? ' — ' + l.nivel_referencia : ''}`;
    }
...
function _lr_renderPanel(el, labIds, dateFrom, dateTo) {
    ...
    el.innerHTML = `
        ...
        <div class="row row-cols-2 row-cols-sm-3 row-cols-lg-5 g-2 mb-4">
            ${_lr_pill('Total indicado',  total,               '#0b1e3d', 'bi-collection')}
            ...
            ${_lr_pill('Completadas',     d.counts.completada, '#00b87a', 'bi-check-circle')}
        </div>

        <!-- ── Exámenes por estado | Resultados por examen ─── -->
        <div class="row g-3 mb-3">
            <div class="col-12 col-lg-8">
                <div class="card border-0 shadow-sm h-100" style="border-radius:12px">
                    <div class="card-body d-flex flex-column">
                        <p class="lr-card-title mb-3">Tipo de examen según estado de la muestra</p>
                        <div id="lr-examenes-wrap"
                             style="position:relative;flex:1;min-height:220px;width:100%"></div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-lg-4">
                <div class="card border-0 shadow-sm h-100" style="border-radius:12px">
                    <div class="card-body">
                        <p class="lr-card-title mb-3">Resultados según tipo de examen</p>
        ...
        <!-- ── Tipos de muestra | Microorganismos ───────────── -->
        <div class="row g-3 mb-3">
            <div class="col-12 col-md-6">
                <div class="card border-0 shadow-sm h-100" style="border-radius:12px">
                    <div class="card-body d-flex flex-column">
                        <p class="lr-card-title mb-1">Tipos de muestra y resultados</p>
            ...
            <div class="col-12 col-md-6">
                <div class="card border-0 shadow-sm h-100" style="border-radius:12px">
                    <div class="card-body d-flex flex-column">
                        <p class="lr-card-title mb-1">Casos positivos según género y especie</p>
                        <p class="mb-3" style="font-size:.75rem;color:#8fa3bf">
                            * BAAR+ (Indeterminado): positivos sin confirmar por cultivo o ensayo Xpert.
                        </p>
                        <div id="lr-species-wrap"
                             style="position:relative;flex:1;min-height:220px;width:100%"></div>
                    </div>
                </div>
            </div>
        ...
        <!-- ── Pirámide poblacional | Grupos de vulnerabilidad ─ -->
        <div class="row g-3 mb-3">
            <div class="col-12 col-md-6">
                <div class="card border-0 shadow-sm h-100" style="border-radius:12px">
                    <div class="card-body">
                        <p class="lr-card-title mb-1">Distribución según edad, sexo y resultado</p>
            ...
            <div class="col-12 col-md-6">
                <div class="card border-0 shadow-sm h-100" style="border-radius:12px">
                    <div class="card-body">
                        <p class="lr-card-title mb-1">Distribución según grupos de vulnerabilidad</p>
        ...
        <!-- ── Resistencia antimicrobiana (ancho completo) ───── -->
        <div class="row g-3">
            <div class="col-12">
                <div class="card border-0 shadow-sm" style="border-radius:12px">
                    <div class="card-body">
                        <p class="lr-card-title mb-1">Resistencia a los medicamentos para la tuberculosis</p>
                        <div id="lr-amr-wrap" class="mb-3"></div>
                        <div style="font-size:.75rem;color:#8fa3bf;line-height:1.7">
                            <p class="mb-1">★ Resistencia a rifampicina: ensayo Xpert MTB/RIF (Ultra). Demás marcadores: Xpert MTB/XDR.</p>
                            <p class="mb-1"><strong>Monorresistencia:</strong> Resistencia a un solo medicamento anti-TB de primera línea.</p>
                            <p class="mb-1"><strong>Polirresistencia:</strong> Resistencia a más de un fármaco anti-TB de primera línea (excluyendo la combinación simultánea rifampicina–isoniazida).</p>
                            <p class="mb-1"><strong>Multidrogorresistencia (MDR):</strong> Resistencia al menos a rifampicina e isoniazida.</p>
                            <p class="mb-1"><strong>Resistencia a Rifampicina (RR):</strong> Detectada por métodos fenotípicos o genotípicos, con o sin resistencia a otros medicamentos anti-TB.</p>
                            <p class="mb-1"><strong>Pre-extensamente resistente (Pre-XDR):</strong> Cepas MDR o RR con resistencia adicional a cualquier fluorquinolona.</p>
                            <p class="mb-0"><strong>Extensamente resistente (XDR):</strong> Cepas MDR o RR con resistencia a fluorquinolonas y al menos un medicamento adicional del grupo A.</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    requestAnimationFrame(() => {
        /* ── Selectores de examen para las 3 cards con filtro ── */
        ...
        _lr_renderPyramidContent(d.pyramid, document.getElementById('lr-pyramid-content'));
        _lr_renderGVContent(d.gvStats, document.getElementById('lr-gv-content'), d.gvCat);
...
function _lr_buildExamFilter(containerId, availableExams, onSelect) {
    ...
    if (!container || availableExams.length <= 1) return;
...
function _lr_computeData(labIds, dateFrom, dateTo) {
    ...
    /* Mapa de recepciones por indicación */
    const recMap = {};
    recs.forEach(r => {
    ...
    /* Tipos de muestra × positivos/negativos (excluye XDR en vista "Todos") */
    const byTMResult = {};
    inds.forEach(ind => {
    ...
    /* Clasificación de farmacorresistencia TB */
    const tbClass = {
        'Sensible': 0, 'Monoresistente': 0, 'Polirresistente': 0,
        'TB-MDR': 0, 'TB pre-XDR': 0, 'TB-XDR': 0,
    };
    inds.forEach(ind => {
    ...
    /* Resistencia antimicrobiana por marcador */
    const amr = {
        total: 0,
        markers: {
    ...
    xpertUF.forEach(r => {
        if (r.resultado === 'MTB DETECTADO') classifyAMR('Rifampicina', r.resistencia_rifampicina);
    });
    xpertXDRF.forEach(r => {
    ...
    /* Pirámide poblacional (positivos/negativos × sexo × edad) */
    ...
    /* Grupos de vulnerabilidad × positivos/negativos */
    const gvStats = {};
    inds.forEach(ind => {
...
function _lr_empty(msg) {
    return `<p class="text-center py-4 mb-0" style="font-size:.84rem;color:#8fa3bf">
        <i class="bi bi-inbox me-1"></i>${msg}
    </p>`;
}

/* ══════════════════════════════════════════════════════════
   RENDERIZADOS (gestionan su propio estado vacío)
   ══════════════════════════════════════════════════════════ */

function _lr_renderResultados(d, el) {
    if (!el) return;
    const exIds = Object.keys(d.resultados).map(Number);
    if (!exIds.length) { el.innerHTML = _lr_empty('Sin resultados.'); return; }
    ...
    let html = exIds.map((eid, i) => {
        ...
        return `<div ${sep}>
            <div class="d-flex align-items-center gap-2 mb-2">
                ...
                <span style="font-size:.82rem;font-weight:600;color:#0b1e3d">
                    ${ex?.nombre || `Examen ${eid}`}
                </span>
    ...
    if (totalRes > 0) {
        const mapColors = {
            'Sensible':       '#00b87a', 'Monoresistente': '#f0a500',
            'Polirresistente':'#f97316', 'TB-MDR':         '#e0435a',
            'TB pre-XDR':     '#be123c', 'TB-XDR':         '#881337',
        };
        let resHtml = `
            <div style="margin-top:1.2rem;padding-top:1rem;border-top:1.5px dashed #cbd5e1">
                <div class="d-flex align-items-center gap-2 mb-2">
                    <span style="font-size:.82rem;font-weight:700;color:#0b1e3d">
                        <i class="bi bi-shield-virus me-1"></i>Perfil de Farmacorresistencia (TB)
                    </span>
        ...
        Object.entries(tc).forEach(([label, cnt]) => {
            ...
            resHtml += `<div class="mb-2">
                ...
                <div style="height:6px;background:#f1f5f9;border-radius:3px">
                    <div style="height:100%;width:${pct}%;background:${mapColors[label]};
                                border-radius:3px;transition:width .4s ease"></div>
                </div>
            </div>`;
        });
...
function _lr_renderPyramidContent(pyramid, el) {
    if (!el) return;
    if (!pyramid || pyramid.total === 0) {
        el.innerHTML = _lr_empty('Sin resultados.');
        return;
    }
            ...
            return `<div class="d-flex align-items-center" style="gap:6px;margin-bottom:6px">
                <div style="flex:1;display:flex;justify-content:flex-end;align-items:center;gap:2px">
                    ...
                    <div style="height:20px;width:${mNPct}%;background:rgba(0,184,122,.85);
                                border-radius:${mP > 0 ? 0 : 3}px 0 0 ${mP > 0 ? 0 : 3}px;
                ...
                <div style="flex:1;display:flex;align-items:center;gap:2px">
                    <div style="height:20px;width:${fNPct}%;background:rgba(0,184,122,.85);
                                border-radius:0 ${fP > 0 ? 0 : 3}px ${fP > 0 ? 0 : 3}px 0;
...
function _lr_renderGVContent(gvStats, el, gvCat) {
    if (!el) return;
    ...
    if (!entries.length) { el.innerHTML = _lr_empty('Sin resultados.'); return; }
        ...
        return `<div style="margin-bottom:.6rem">
            <div class="d-flex justify-content-between" style="font-size:.75rem;margin-bottom:3px">
                ...
                <span style="font-family:'IBM Plex Mono',monospace;white-space:nowrap;margin-left:.4rem">
                    <span style="color:#e0435a;font-weight:700">${e.pos}</span>
                    <span style="color:#ccc;font-size:.65rem">+</span>
                    <span style="color:#00b87a;font-weight:700;margin-left:.2rem">${e.neg}</span>
                    <span style="color:#ccc;font-size:.65rem">−</span>
                </span>
            </div>
...
/* ══════════════════════════════════════════════════════════
   GRÁFICOS (gestionan su propio estado vacío)
   ══════════════════════════════════════════════════════════ */

function _lr_chartExamenes(d) {
    const wrap = document.getElementById('lr-examenes-wrap');
    if (!wrap) return;
    ...
    if (!examIds.length) {
        wrap.innerHTML = _lr_empty('Sin exámenes en el período.');
        return;
    }

    wrap.innerHTML = '<canvas id="lr-c-examenes"></canvas>';
    const c = document.getElementById('lr-c-examenes');

    const labels = examIds.map(id => _EXAMENES_CAT.find(e => e.id === id)?.nombre || `Examen ${id}`);
    const mk = (label, key, bg) => ({
        label, backgroundColor: bg, borderRadius: 3, maxBarThickness: 32,
...
function _lr_chartTM(d) {
    const wrap = document.getElementById('lr-tm-content');
    if (!wrap) return;
    ...
    if (!entries.length) {
        wrap.innerHTML = _lr_empty('Sin resultados.');
        return;
    }

    if (_hl_charts['lr-tm']) { _hl_charts['lr-tm'].destroy(); delete _hl_charts['lr-tm']; }
    wrap.innerHTML = '<canvas id="lr-c-tm"></canvas>';
...
function _lr_chartSpecies(d) {
    const wrap = document.getElementById('lr-species-wrap');
    if (!wrap) return;

    const entries = Object.entries(d.microorganisms.data)
        .sort((a, b) => b[1] - a[1]);
    ...
    wrap.innerHTML = '<canvas id="lr-c-species"></canvas>';
    const c = document.getElementById('lr-c-species');

    _hl_charts['lr-species'] = new Chart(c, {
        type: 'bar',
        data: {
            labels: entries.map(e => e[0].length > 38 ? e[0].slice(0, 35) + '…' : e[0]),
            datasets: [{
                label: 'Casos aislados',
...
function _lr_chartAMR(d) {
    const wrap = document.getElementById('lr-amr-wrap');
    if (!wrap) return;
    ...
    wrap.innerHTML = `<div style="position:relative;height:${h}px;width:100%">
        <canvas id="lr-c-amr"></canvas>
    </div>`;
    const c = document.getElementById('lr-c-amr');
