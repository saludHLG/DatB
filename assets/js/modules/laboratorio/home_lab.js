/* =========================================================
   ...
   Sin localStorage. Requiere: utils.js · data.js · laboratorio_core.js
   ========================================================= */
...
function _hl_renderContent(user, el, dateFrom, dateTo) {
    ...
    <style>
        .hl-card-title { font-family:'Syne',sans-serif; font-size:.88rem; font-weight:700; color:#0b1e3d; margin-bottom:0; }
    </style>`;
...
function _hl_computeData(user, dateFrom, dateTo) {
    ...
    /* ── Fuentes de datos desde _store (sin localStorage) ── */
    ...
    inds.forEach(ind => {
        (ind.examenes_ids || []).forEach(eidRaw => {
            ...
            const rec = recs.find(r => r.indicacion_id === ind.id && Number(r.examen_id) === eid)
                     || recs.find(r => r.indicacion_id === ind.id && !r.examen_id)
                     || null;
            let est;
            if (!rec)                        est = 'pendiente';
            else if (rec.estado === 'rechazada') est = 'rechazada';
            else est = _hl_hasResult(rec.id, eid, baci, cult, xpertU, xpertXDR) ? 'completada' : 'recibida';
            counts[est]++;
    ...
    if (xpertUF.length)   { resultados[3] = {}; xpertUF.forEach(r => { resultados[3][r.resultado] = (resultados[3][r.resultado] || 0) + 1; }); }
    if (xpertXDRF.length) { resultados[5] = {}; xpertXDRF.forEach(r => { resultados[5][r.resultado] = (resultados[5][r.resultado] || 0) + 1; }); }
    ...
    inds.forEach(ind => {
        ...
        (ind.examenes_ids || []).forEach(eidRaw => {
            const eid = Number(eidRaw);
            const rec = recs.find(r => r.indicacion_id === ind.id && (Number(r.examen_id) === eid || !r.examen_id));
            if (!rec || rec.estado !== 'recibida') return;
            if (eid === 1) { const r = baci.find(x => x.recepcion_id === rec.id); if (r) { if (r.codificacion > 0) hasPos = true; else hasNeg = true; } }
            else if (eid === 2) { const r = cult.find(x => x.recepcion_id === rec.id); if (r && r.resultado !== 'en_estudio' && r.resultado !== 'contaminado') { if (r.resultado === '0') hasNeg = true; else hasPos = true; } }
            else if (eid === 3) { const r = xpertU.find(x => x.recepcion_id === rec.id); if (r) { if (r.resultado === 'MTB DETECTADO') hasPos = true; else if (r.resultado === 'MTB NO DETECTADO') hasNeg = true; } }
            else if (eid === 5) { const r = xpertXDR.find(x => x.recepcion_id === rec.id); if (r) { if (r.resultado === 'MTB DETECTADO') hasPos = true; else if (r.resultado === 'MTB NO DETECTADO') hasNeg = true; } }
        });
...
function _hl_getGVCat() {
    ...
    return window._store.grupos_vulnerables || [];
}

function _hl_getTMCat() {
    if (typeof _getTMCat === 'function') return _getTMCat();
    return window._store.tipos_muestra || [];
}
...
function _hl_chartEstados(d) {
    const c = document.getElementById('hl-c-estados'); if (!c) return;
...
function _hl_chartExamenes(d) {
    const c = document.getElementById('hl-c-examenes'); if (!c) return;
...
function _hl_chartTM(d) {
    const c = document.getElementById('hl-c-tm'); if (!c) return;
    const entries = Object.entries(d.byTM).map(([id, n]) => ({ label: d.tmCat.find(m => m.id === Number(id))?.nombre || `Muestra #${id}`, n })).sort((a,b) => b.n-a.n).slice(0,12);
    _hl_charts.tm = new Chart(c, {
        type: 'bar',
        data: { labels: entries.map(e=>e.label), datasets:[{ label:'Indicaciones', data:entries.map(e=>e.n), backgroundColor:'#00c6b8', borderRadius:3 }] },
        options: { indexAxis:'y', responsive:true, plugins:{legend:{display:false}}, scales:{ x:{ticks:{precision:0,color:'#64748b'},grid:{color:'rgba(0,0,0,.05)'}}, y:{ticks:{font:{size:11},color:'#334155'},grid:{display:false}} } }
    });
}
...
function _hl_renderResultados(d, el) {
    ...
    if (!exIds.length) { el.innerHTML = _hl_empty('Sin resultados emitidos en el período.'); return; }
...
function _hl_chartPiramide(d) {
    const c = document.getElementById('hl-c-piramide'); if (!c) return;
    _hl_charts.piramide = new Chart(c, {
        type: 'bar',
        data: { labels: d.pyramid.ageGroups, datasets: [
            { label:'Masculino', data: d.pyramid.M.map(v=>-v), backgroundColor:'rgba(26,86,219,.72)', borderRadius:3 },
            { label:'Femenino',  data: d.pyramid.F,             backgroundColor:'rgba(224,67,90,.72)', borderRadius:3 }
        ]},
        options: { indexAxis:'y', responsive:true, scales:{ x:{ ticks:{ callback:v=>Math.abs(v), precision:0, color:'#64748b' }, grid:{color:'rgba(0,0,0,.05)'} }, y:{grid:{display:false},ticks:{color:'#334155'}} }, plugins:{ legend:{position:'bottom',labels:{font:{size:11},padding:10,boxWidth:12,color:'#334155'}}, tooltip:{callbacks:{label:ctx=>`${ctx.dataset.label}: ${Math.abs(ctx.raw)}`}} } }
    });
}

function _hl_chartGV(d) {
    const c = document.getElementById('hl-c-gv'); if (!c) return;
    const entries = Object.entries(d.gvStats).map(([id,v]) => ({ label: d.gvCat.find(g=>g.id===Number(id))?.nombre||`Grupo ${id}`, pos:v.pos, neg:v.neg })).filter(e=>e.pos+e.neg>0).sort((a,b)=>(b.pos+b.neg)-(a.pos+a.neg)).slice(0,15);
    _hl_charts.gv = new Chart(c, {
        type: 'bar',
        data: { labels: entries.map(e=>e.label.length>42?e.label.slice(0,39)+'…':e.label), datasets: [
            { label:'Positivos', data:entries.map(e=>e.pos), backgroundColor:'rgba(224,67,90,.8)', borderRadius:3 },
            { label:'Negativos', data:entries.map(e=>e.neg), backgroundColor:'rgba(0,184,122,.8)', borderRadius:3 }
        ]},
        options: { indexAxis:'y', responsive:true, scales:{ x:{ticks:{precision:0,color:'#64748b'},grid:{color:'rgba(0,0,0,.05)'}}, y:{grid:{display:false},ticks:{font:{size:10},color:'#334155'}} }, plugins:{ legend:{position:'bottom',labels:{font:{size:11},padding:10,boxWidth:12,color:'#334155'}} } }
    });
}
