...
(() => {
    ...
    const AUTH_FUNCTION = 'datb-auth-prod';
    ...
    async function errorDetail(error, fallback) {
    ...
    window.sbLogin = async function (ci, pin) {
        ...
        if (error) return { user: null, error: await errorDetail(error, 'Error de autenticación.') };
    ...
    window.sbRegister = async function (perfil, pin) {
        ...
        if (error) return { error: await errorDetail(error, 'No se pudo registrar la cuenta.') };
    ...
    window.sbChangePin = async function (userId, newPin) {
        ...
        if (error) return { error: await errorDetail(error, 'No se pudo cambiar el PIN.') };
        if (data?.error) return { error: data.stage ? `${data.error} [${data.stage}]` : data.error };
        return { error: null };
    };
})();
